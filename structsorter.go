// Package structsorter defines an Analyzer that detects structs that would use less
// memory if their fields were sorted.
package structsorter
...
func run(pass *analysis.Pass) (interface{}, error) {
	...
	inspect.Preorder(nodeFilter, func(node ast.Node) {
		...
		if tv, ok := pass.TypesInfo.Types[s]; ok {
			structSorter(pass, s, tv.Type.(*types.Struct))
		}
	})
	return nil, nil
}
...
func structSorter(pass *analysis.Pass, node *ast.StructType, typ *types.Struct) {
...
func optimalOrder(str *types.Struct, sizes *gcSizes) (*types.Struct, []int) {
	...
	sort.Slice(elems, func(i, j int) bool {
		...
		if ei.sizeof == ej.sizeof {
			return strings.Compare(ei.field, ej.field) == -1
		}
